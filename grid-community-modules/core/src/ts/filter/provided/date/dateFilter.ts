...
export class DateFilter extends ScalarFilter<DateFilterModel, Date, DateCompWrapper> {
    ...
    private isValidDateValue(value: Date | null): boolean {
        ...
        if (this.minValidDate instanceof Date) {
            if (value < this.minValidDate) {
                return false;
            } else {
                // Condition 1 met: Value is greater than minValidDate
            }
        } else {
            if (value.getUTCFullYear() < this.minValidYear) {
                return false;
            } else {
                // Condition 1 met: Value is greater than minValidYear
            }
        }

        if (this.maxValidDate instanceof Date) {
            if (value > this.maxValidDate) {
                return false;
            } else {
                // Condition 2 met: Value is less than maxValidDate
            }
        } else {
            if (value.getUTCFullYear() > this.maxValidYear) {
                return false;
            } else {
                // Condition 2 met: Value is less than maxValidYear
            }
        }

        // Conditions 1 and 2 met: Value is within min and max
        return true;
    };
